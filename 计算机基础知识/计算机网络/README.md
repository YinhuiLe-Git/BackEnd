...
* 为什么TIME-WAIT状态必须等待2MSL时间？
    ...
    * 时间MSL叫做最长报文段寿命，经过2MSL后，可以使得本次连接的时间内所产生的所有报文都从网络中消失。这样可以使得下一个新的连接中不会出现旧的连接请求报文段。 
...
* 迭代查询
    * 当根域名服务器收到本地域名服务器发出的迭代查询请求报文时，要么给出IP地址，要么告诉本地域名服务器，应该向哪一个域名服务器进行查询，然后本地域名服务器进行后续查询(而不是替本地域名服务器进行后续的查询) 
...
* TCP的数据包然后会发送给IP层，用到**IP协议**。IP层查询路由表以决定如何发送该IP数据报，IP数据报首先被发送至路由器，再由路由器进行转发。若本地的ARP缓存中没有路由器的MAC地址，需要发起ARP广播以查询路由器MAC地址，以太网驱动程序将IP数据报封装成以太网帧发送给路由器。
* 当然在一个网段内的寻址是通过以太网协议实现，**以太网协议**需要知道目的IP地址的物理地址，则需要**ARP协议**。虽然IP数据报是先发送到路由器再进行转发给目标主机，但是其头部的目标IP地址和源端IP地址是始终不变的，但帧头部的源端物理地址和目的端物理地址在转发过程中是一直变化的。
